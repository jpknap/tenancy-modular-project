...
El argumento completo del usuario llega en `$ARGUMENTS`. Interpreta el comando según el primer token:

### `/feature` o `/feature board`
Muestra el tablero completo formateado en markdown con todas las columnas y sus cards.

### `/feature add "título" [--project=X] [--priority=X] [--col=X] [--desc="..."]`
...
7. Confirma la card creada

### `/feature move <id> <columna>`
1. Lee `.kanban/BOARD.md`
2. Busca la card por ID (ej: `#003` o `003`)
3. La mueve a la columna indicada
4. Si la columna destino es `done`, cambia `[ ]` por `[x]`
5. Guarda y confirma

### `/feature done <id>`
Alias de `/feature move <id> done`

### `/feature list [--col=X] [--project=X] [--priority=X]`
Lista cards filtrando por columna, proyecto o prioridad. Si no hay filtros, lista todo.

### `/feature remove <id>`
Elimina la card del tablero y confirma.

### `/feature show <id>`
Muestra el detalle completo de una card.

## Comportamiento general

- Si `.kanban/BOARD.md` no existe, créalo con la estructura vacía antes de operar.
- Siempre muestra el resultado de la operación de forma clara y concisa.
- Al mostrar el board, usa tablas markdown por columna con colores/emojis para prioridad:
  - 🔴 high  🟡 medium  🟢 low
